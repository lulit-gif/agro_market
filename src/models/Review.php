...
class Review
{
    ...
    public static function create(int $userId, int $productId, int $rating, string $comment): void
    {
        $pdo = get_db();
        $stmt = $pdo->prepare('INSERT INTO reviews (user_id, product_id, rating, comment, created_at)
                               VALUES (?, ?, ?, ?, NOW())');
